...
class NodeTemplateRelationshipTarget(Element):
    ...
    def validate(self):
        ...
        node_name = self.ancestor(NodeTemplate).name
        node_template_names = list(self.ancestor(NodeTemplates).initial_value.keys())
...
def _process_operations(partial_error_message,
                        ...
                        resource_base):
    operations = {}
    for interface_name, interface in list(interfaces.items()):
    ...
    return dict((operation_name, operation) for operation_name, operation in
                list(operations.items()) if operation is not None)
...
def _process_nodes_plugins(processed_nodes,
    ...
    nodes_operations = dict((name, []) for name in list(processed_nodes.keys()))
    for node_name, node in list(processed_nodes.items()):
    ...
    for node in list(processed_nodes.values()):
        # set plugins_to_install property for nodes
        if node['type'] in host_types:
            plugins_to_install = {}
            for another_node in list(processed_nodes.values()):
            ...
            node[constants.PLUGINS_TO_INSTALL] = list(plugins_to_install.values())
